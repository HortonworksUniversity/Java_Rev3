...
CID_namenode=$(docker run -d --privileged --dns 8.8.8.8 -p 50070:50070 -p 8020:8020 -p 8080:8080 -p 9080:9080 -e NODE_TYPE=namenode --name namenode -h namenode -i -t hwx/hdp_storm_node)
...
CID_hive=$(docker run -d --privileged --link namenode:namenode -e namenode_ip=$IP_namenode -e NODE_TYPE=hiveserver --dns 8.8.8.8 -p 50075 -p 50010 -p 50020 -p 8010 -p 45454 -p 11000:11000 -p 2181 -p 50111:50111 -p 9083 -p 10000 -p 9999:9999 -p 9933:9933 -p 22  --name hiveserver -h hiveserver -i -t hwx/hdp_storm_node)
...
do
nodename="node$i"
CID=$(docker run -d --privileged --link namenode:namenode -e namenode_ip=$IP_namenode -e NODE_TYPE=workernode -e LOG_PORT=908$i --dns 8.8.8.8 -p 8010 -p 50075 -p 50010 -p 50020 -p 45454 -p 8081 -p 22 -p 908$i --name $nodename -h $nodename -i -t hwx/hdp_storm_node)
